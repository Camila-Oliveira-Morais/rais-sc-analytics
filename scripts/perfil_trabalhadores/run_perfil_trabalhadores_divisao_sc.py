...
LOGGER = logging.getLogger("perfil_trabalhadores_divisao_sc")
...
def resolve_columns(file_path: Path, separator: str) -> dict[str, str]:
    ...
    required_columns = {
        ...
        "sexo_codigo": ["sexo - codigo", "sexo trabalhador"],
        "escolaridade_codigo": ["escolaridade apos 2005 - codigo", "escolaridade apos 2005"],
        "idade": ["idade"],
...
def aggregate_chunk(
    ...
    chunk["idade"] = pd.to_numeric(chunk["idade"], errors="coerce").astype("Int64")
    ...
    chunk["cnae_subclasse_codigo"] = chunk["cnae_subclasse_codigo"].apply(lambda value: clean_digits(value, width=7))
    chunk["sexo_codigo"] = chunk["sexo_codigo"].apply(lambda value: clean_digits(value))
    chunk["escolaridade_codigo"] = chunk["escolaridade_codigo"].apply(lambda value: clean_digits(value))
    ...
    filtered = chunk.loc[
        ...
        & chunk["idade"].notna()
        & chunk["sexo_codigo"].notna()
        & chunk["escolaridade_codigo"].notna()
    ].copy()
    ...
    filtered["idade"] = filtered["idade"].astype("int64")
    ...
    enriched["sexo_nome"] = enriched["sexo_codigo"].map(SEX_LABELS).fillna("Sexo nao identificado")
    enriched["escolaridade_nome"] = enriched["escolaridade_codigo"].map(EDUCATION_LABELS).fillna(
        "Escolaridade nao identificada"
    )
...
def aggregate_to_mesorregiao(detail: pd.DataFrame) -> pd.DataFrame:
    aggregated = (
        detail.groupby(
            [
                "ano_referencia",
...
def choose_output_geography(detail: pd.DataFrame) -> tuple[pd.DataFrame, str, str]:
    if len(detail) <= EXCEL_MAX_ROWS:
        return detail, "municipio", "Detalhamento por municipio mantido dentro do limite do Excel."

    mesorregiao_detail = aggregate_to_mesorregiao(detail)
    return (
        mesorregiao_detail,
        "mesorregiao",
        "Detalhamento por municipio excedeu o limite do Excel; arquivo final agregado por mesorregiao.",
    )
...
def build_metadata(
    ...
    geography: str,
    municipality_row_count: int,
    final_row_count: int,
    stats_rows: list[dict[str, int | str]],
    geography_reason: str,
) -> pd.DataFrame:
    metadata_rows: list[dict[str, object]] = [
        {"chave": "anos_solicitados", "valor": ", ".join(requested_years)},
        {"chave": "anos_processados", "valor": ", ".join(processed_years)},
        ...
        {"chave": "geografia_final", "valor": geography},
        {"chave": "linhas_municipio_estimadas", "valor": municipality_row_count},
        {"chave": "linhas_arquivo_final", "valor": final_row_count},
        {"chave": "motivo_geografia_final", "valor": geography_reason},
        {"chave": "perfil_colunas", "valor": "sexo, escolaridade apos 2005, idade e CNAE divisao"},
    ]

    for skipped in skipped_years:
        metadata_rows.append(
            {
                "chave": f"ano_indisponivel_{skipped['ano_referencia']}",
                "valor": skipped["motivo"],
            }
        )
...
def auto_fit_columns(writer: pd.ExcelWriter, sheet_name: str, dataframe: pd.DataFrame) -> None:
    ...
    for index, column in enumerate(dataframe.columns):
        ...
        applied_format = integer_format if column in {"ano_referencia", "municipio_codigo", "idade", "qtd_vinculos"} else None
        worksheet.set_column(index, index, min(max_length + 2, 40), applied_format)


def build_default_output_path(output_dir: Path, processed_years: list[str], geography: str) -> Path:
    if len(processed_years) == 1:
        suffix = processed_years[0]
    else:
        suffix = f"{processed_years[0]}_{processed_years[-1]}"
    return output_dir / f"perfil_trabalhadores_sc_cnae_divisao_{geography}_{suffix}.xlsx"


def export_workbook(detail: pd.DataFrame, summary: pd.DataFrame, metadata: pd.DataFrame, output_path: Path) -> Path:
    detail_sheet = "Perfil_Municipio" if "municipio_codigo" in detail.columns else "Perfil_Mesorregiao"
    with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
        ...
        auto_fit_columns(writer, detail_sheet, detail)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Gera perfil de trabalhadores da RAIS vinculo para SC por CNAE divisao."
    )
    parser.add_argument(
        "--years",
        nargs="+",
...
def main() -> None:
    ...
    requested_years = [str(year) for year in args.years]
    ...
    for year in requested_years:
        ...
        try:
            ...
            skipped_years.append({"ano_referencia": year, "motivo": str(error)})
            LOGGER.warning("Ano %s ignorado: %s", year, error)
            continue
    ...
    if not yearly_frames:
        raise ValueError("Nenhum ano disponivel para processamento com os filtros solicitados.")

    municipality_detail = pd.concat(yearly_frames, ignore_index=True).sort_values(
        [
            "ano_referencia",
            "municipio_nome",
            "cnae_divisao_codigo",
    ...
    final_detail, geography, geography_reason = choose_output_geography(municipality_detail)
    summary = build_summary(municipality_detail)
    metadata = build_metadata(
        requested_years=requested_years,
        processed_years=processed_years,
        skipped_years=skipped_years,
        geography=geography,
        municipality_row_count=len(municipality_detail),
        final_row_count=len(final_detail),
    ...
    output_path = (
        ...
        if args.output_path
        else build_default_output_path(output_dir, processed_years, geography)
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    export_workbook(final_detail, summary, metadata, output_path)
